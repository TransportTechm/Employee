...
var _OAuth2Util = require('./OAuth2Util');

var _OAuth2Util2 = _interopRequireDefault(_OAuth2Util);
...
var Eureka = function (_EventEmitter) {
    ...
    function Eureka() {
        ...
        var filename = config.filename || 'pcf-eureka-client';
        ...
        if (_this.oAuth2Enabled) {
            _this.oAuth2Util = new _OAuth2Util2.default(_this.config.oauth2.clientCredentials);
        }
        return _this;
    }
    ...
    Eureka.prototype.register = function register() {
        ...
        this.lookupCurrentEurekaHost(function (err, eurekaUrl, token) {
            ...
            _this3.setAuthorizationInHeader(requestData, token);
    ...
    Eureka.prototype.deregister = function deregister() {
        ...
        this.lookupCurrentEurekaHost(function (err, eurekaUrl, token) {
            ...
            _this4.setAuthorizationInHeader(requestData, token);

            _request2.default.del(requestData, function (error, response, body) {
    ...
    Eureka.prototype.renew = function renew() {
        ...
        this.lookupCurrentEurekaHost(function (err, eurekaUrl, token) {
            ...
            _this6.setAuthorizationInHeader(requestData, token);

            _request2.default.put(requestData, function (error, response, body) {
                if (!error && response.statusCode === 200) {
                    ...
                    if (error) {
                        _this6.logger.error('An error in the request occurred.', error);
                    }
    ...
    Eureka.prototype.fetchRegistry = function fetchRegistry() {
        ...
        this.lookupCurrentEurekaHost(function (err, eurekaUrl, token) {
            ...
            _this8.setAuthorizationInHeader(requestData, token);
    ...
    Eureka.prototype.lookupCurrentEurekaHost = function lookupCurrentEurekaHost() {
        ...
        if (this.config.eureka.useDns) {
            this.locateEurekaHostUsingDns(function (err, resolvedHost) {
                ...
                if (_this12.oAuth2Enabled) {
                    _this12.oAuth2Util.getTokenForClientCredentials(function (error, token) {
                        if (error) {
                            _this12.logger.error('An error in the request occurred.', error);
                        }

                        return callback(null, _url2.default.format(parsed), token);
                    });
                } else {
                    return callback(null, _url2.default.format(parsed), null);
                }
            });
        } else {
            if (this.oAuth2Enabled) {
                this.oAuth2Util.getTokenForClientCredentials(function (error, token) {
                    if (error) {
                        _this12.logger.error('An error in the request occurred.', error);
                    }

                    return callback(null, _this12.currentServiceUrl, token);
                });
            } else {
    ...
    Eureka.prototype.setAuthorizationInHeader = function setAuthorizationInHeader(requestData, token) {
        if (!token) {
            return;
        }
    ...
    _createClass(Eureka, [{
        ...
        key: 'oAuth2Enabled',
        get: function get() {
            return this.config.oauth2 && this.config.oauth2.clientCredentials;
        }
    }]);
